...
class Chime7Lhotse(datasets.GeneratorBasedBuilder):
    ...
    def _fetch_split_meta(self, split: list):
        ...
        for s in split:
            ...
            recordings = load_gzjson(Path(metadata_path,'{}_selected_recordings.jsonl.gz'.format(s.replace('-','_'))))
            recordings_metadata.update({x['id']: x for x in recordings})
            split_metadata = {s:load_gzjson(Path(metadata_path,'{}_selected_supervisions.jsonl.gz'.format(s.replace('-','_'))))}
                    ...
                    for si in supervisions_metadata[key]:
                        ...
                        for c in si['channel']:
                            ...
                            for s in recordings_metadata[recording_id]['sources']:
                                if s['channels'] == [c]:
                                    wav_i, fs = sf.read(s['source'],start=int(si['start']*fs),stop=int((si['start']+si['duration'])*fs))
                                    si_out.append(wav_i)
